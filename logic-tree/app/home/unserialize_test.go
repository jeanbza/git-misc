...
func TestUnserializeTreeArbitraryWidth(t *testing.T) {
    ...
    treeReturned, errorsReturned := unserializeTree(testingTreeConditions)

    if !treeReturned.matches(testingTreeRoot) {
        t.Errorf("unserializeTree(%v) - got %v, want %v", testingTreeConditions, treeReturned.print(), testingTreeRoot.print())
    }

    var expectedOutErr error
    if errorsReturned != expectedOutErr {
        t.Errorf("unserializeTree(%v) errorsReturned - got %v, want %v", testingTreeConditions, errorsReturned, expectedOutErr)
    }
}