...
func TestSerializeTreeArbitraryWidth(t *testing.T) {
    ...
    if !matchesArray(conditionsReturned, testingTreeConditions) {
        t.Errorf("serializeTree(%v) conditionsReturned - got %v, want %v", testingTreeRoot, simplifyConditions(conditionsReturned), simplifyConditions(testingTreeConditions))
    }
